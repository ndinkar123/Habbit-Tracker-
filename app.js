...
// ---------- SVG SANITIZER (for custom habit-icon import) ----------
// Strict allowlist: only safe shape/paint elements & attributes survive.
// Strips <script>, <foreignObject>, <style>, <image>, <use>, event-handler
// attributes (on*), inline style attributes, and any non-fragment href.
const SVG_ALLOWED_TAGS = new Set([
  'svg', 'path', 'circle', 'rect', 'line', 'polyline', 'polygon', 'g',
  'ellipse', 'defs', 'lineargradient', 'radialgradient', 'stop', 'clippath', 'mask'
]);
const SVG_ALLOWED_ATTRS = new Set([
  ...
  'clip-rule', 'd', 'cx', 'cy', 'r', 'rx', 'ry', 'x', 'y', 'x1', 'y1', 'x2', 'y2',
  'points', 'transform', 'opacity', 'offset', 'stop-color', 'stop-opacity',
  'id', 'gradientunits', 'gradienttransform', 'xmlns'
]);
...
function sanitizeSvgString(raw) {
  ...
  function cleanAttrs(el) {
    for (const attr of Array.from(el.attributes)) {
      ...
      if (name === 'href' || name === 'xlink:href') { el.removeAttribute(attr.name); continue; }
  ...
  cleanAttrs(svgEl);
  clean(svgEl);
  svgEl.querySelectorAll('script, foreignobject, style, image, iframe, use, a').forEach(n => n.remove());
  ...
  try {
    ...
    return out.length <= 20000 ? out : null;
  } catch (e) { return null; }
}
...
// Sentinel "always existed" date used for default/legacy habits so the
// createdAt-based historical filtering below never breaks old data.
const LEGACY_DATE = '2000-01-01';

const DEFAULT_HABITS = [
  { id: generateSafeId(), name: 'Read 30 Mins', emoji: '📚', icon: 'book', createdAt: LEGACY_DATE },
  { id: generateSafeId(), name: 'Exercise', emoji: '💪', icon: 'dumbbell', createdAt: LEGACY_DATE },
  { id: generateSafeId(), name: 'Meditate', emoji: '🧘', icon: 'meditation', createdAt: LEGACY_DATE }
];
...
function sanitizeState(raw) {
  ...
  if (Array.isArray(raw.habits)) {
    ...
    for (const h of raw.habits) {
      ...
      // Habits saved before this field existed are treated as "always existed"
      // so their historical stats aren't retroactively changed.
      const createdAt = (typeof h.createdAt === 'string' && DATE_RE.test(h.createdAt)) ? h.createdAt : LEGACY_DATE;
      // Re-sanitize on every load/import — never trust stored/imported markup as-is.
      const customIcon = (typeof h.customIcon === 'string') ? sanitizeSvgString(h.customIcon) : null;
      const habit = { id, name, emoji, createdAt };
      if (customIcon) habit.customIcon = customIcon;
      else if (icon) habit.icon = icon;
      s.habits.push(habit);
    }
  }
  ...
  s.pinEnabled = !!raw.pinEnabled;
  s.pinHash = (typeof raw.pinHash === 'string') ? raw.pinHash.slice(0, 64) : '';
  if (s.pinEnabled && !s.pinHash) s.pinEnabled = false; // never lock with no way to unlock
...
// Habits can be checked for today AND yesterday (a one-day grace window),
// so forgetting to log at night can still be fixed the next morning —
// but nothing older than that can be edited.
function isEditableSelectedDate() {
  const now = getNow();
...
function renderHabitGlyph(h) {
  if (h.customIcon) return h.customIcon;
...
// A habit only counts toward a day's total if it existed on/before that day —
// otherwise adding a new habit today would retroactively lower every past
// day's completion percentage (streaks, heatmap, month average, etc).
function habitExistedOn(h, k) {
  return !h.createdAt || h.createdAt <= k;
}
...
function buildTodayView() {
  ...
  const locked = !isEditableSelectedDate();
...
async function toggleHabit(habitId) {
  if (!isEditableSelectedDate()) {
    showToast('🔒 Only today and yesterday can be edited');
    return;
  }
  const key = dateKey(currentYear, currentMonth, selectedDate);
...
function buildProgressView() {
  ...
  // Denominator is the FULL month (not just days elapsed) so each row is a
  // true monthly progress bar — it starts near-empty and fills in as days
  // are completed, rather than swinging wildly based on today's date.
  // Days before a habit's createdAt don't count against it either way.
  const daysInMonthTotal = new Date(currentYear, currentMonth + 1, 0).getDate();

  const list = els.perfRows;
  list.innerHTML = '';
  const frag = document.createDocumentFragment();
  let sumDone = 0, sumEligible = 0;
  state.habits.forEach((h, idx) => {
    if (!h || !h.id) return;
    let done = 0, eligible = 0;
    for (let i = 1; i <= daysInMonthTotal; i++) {
      const k = dateKey(currentYear, currentMonth, i);
      if (!habitExistedOn(h, k)) continue;
      eligible++;
      if (getDayData(k)[h.id]) done++;
    }
    const pct = eligible > 0 ? Math.round((done / eligible) * 100) : 0;
    sumDone += done; sumEligible += eligible;
    ...
    row.innerHTML =
      ...
      '<div class="perf-info">' +
        ...
        '<div class="perf-frac">' + done + '/' + eligible + ' days this month</div>' +
      '</div>' +
  ...
  if (els.overallMonthPct) {
    els.overallMonthPct.textContent = (sumEligible > 0 ? Math.round((sumDone / sumEligible) * 100) : 0) + '%';
  }
}
...
// Badge checking is standalone so it can run from any action that might
// unlock one (habit toggle, adding a habit, completing a task) — previously
// it only ran on habit toggle, so "Builder" / "Task Crusher" unlocked late.
function checkBadgesFor(dayKeyForStats) {
  const key = dayKeyForStats || dateKeyFromDate(getNow());
  const stats = calculateDayStats(key);
...
async function addNewHabit() {
  state.habits.push({
    id: generateSafeId(), name: 'New Habit', emoji: '🌟', icon: 'target',
    createdAt: dateKeyFromDate(getNow())
  });
  checkBadgesFor();
  await saveState();
  buildSettingsView();
...
// Full wipe with no further confirmation — used by the "Forgot PIN" recovery
// flow, which already confirms with the person before calling this.
...
function cacheEls() {
  const ids = [
    ...
    'iconPicker','iconCategoryTabs','iconGrid','iconUploadBtn','iconUploadInput','toast',
    'updateBanner','updateRefreshBtn','exportModal','exportClose','exportSummary',
    'exportConfirmBtn','overallMonthPct'
  ];
...
function bindEvents() {
  ...
  els.iconGrid.addEventListener('click', async (e) => {
    ...
    if (h) {
      h.icon = btn.dataset.icon;
      delete h.customIcon;
      await saveState(); buildSettingsView(); buildTodayView();
    }
    closeIconPicker();
  });

  els.iconUploadBtn.addEventListener('click', () => els.iconUploadInput.click());
  els.iconUploadInput.addEventListener('change', async (e) => {
    const file = e.target.files[0];
    e.target.value = '';
    if (!file || !activeEditingHabitId) return;
    if (file.size > 20000) { showToast('⚠️ SVG file too large (max 20KB)'); return; }
    if (!/\.svg$/i.test(file.name) && file.type !== 'image/svg+xml') {
      showToast('⚠️ Please choose an .svg file');
      return;
    }
    let text;
    try { text = await file.text(); } catch (err) { showToast('⚠️ Could not read that file'); return; }
    const clean = sanitizeSvgString(text);
    if (!clean) { showToast('⚠️ That SVG could not be used'); return; }
    const h = state.habits.find(x => x.id === activeEditingHabitId);
    if (h) {
      h.customIcon = clean;
      delete h.icon;
      await saveState(); buildSettingsView(); buildTodayView();
      showToast('✅ Custom icon set');
    }
    closeIconPicker();
  });
  ...
  // Tapping the dimmed backdrop closes any standard modal (previously only
  // the icon picker supported this — now it's consistent everywhere except
  // the PIN lock screen, which is intentionally not backdrop-dismissible).
  document.querySelectorAll('.modal-overlay').forEach(overlay => {
    if (overlay.id === 'iconOverlay') return; // already has its own handler
...
async function init() {
  ...
  if (window.PinModule) { PinModule.init(); PinModule.maybeLockOnBoot(() => {}); }
