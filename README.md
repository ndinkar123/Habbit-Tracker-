...
✅ **4-digit App Lock** — optional PIN lock, toggle on/off and change it from Settings → Security. Stored as a hash, never in plain text. "Forgot PIN" is available on the lock screen but wipes local data (no server means no recovery).

✅ **Edit window widened to today + yesterday** — forgot to check off a habit last night? You can still mark it the next day. Anything older stays locked.

✅ **Custom SVG icons** — upload your own `.svg` icon per habit from the icon picker (in addition to the 36 built-in icons). Uploaded SVGs are sanitized (scripts/handlers stripped) before use.

✅ **Fixed:** adding a new habit no longer retroactively lowers past days' completion % — habits now only count toward days on/after they were created.

✅ **Fixed:** Progress tab habit rows now show real monthly progress (days done ÷ full days in month) instead of a number that swung wildly early in the month, plus a new "Overall Progress This Month" summary.

✅ **Fixed:** "Builder" and "Task Crusher" badges now unlock immediately when earned, instead of only being checked on your next habit tap.

✅ **Fixed:** the "New version available" banner no longer races with the service worker auto-activating.

✅ **Fixed:** toast and update-banner no longer overlap; all modals now close on backdrop tap; unused legacy CSS removed; hardened BMI modal layout against narrow-screen overflow.
...
Place all 11 files in your GitHub Pages repository root:
...
├── pin.js                # 4-digit App Lock module
...
Script load order matters — `index.html` already loads them correctly (`storage.js` → `icons.js` → `todo.js` → `bmi.js` → `pin.js` → `app.js`). Don't reorder the `<script>` tags if you edit the HTML.
